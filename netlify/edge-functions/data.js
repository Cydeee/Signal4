...
async function buildDashboardData() {
  const SYMBOL = "BTCUSDT"; // make this parametric later for any top‑20 symbol
  const LIMIT = 250; // enough history for MACD (26+9) and EMA200
  const result = {
    dataA: {},
    dataB: {},
    dataC: {},
  ...
  // MACD 12‑26‑9 helper
  const macd = (a, fast = 12, slow = 26, signal = 9) => {
    if (a.length < slow + signal) return { line: 0, signal: 0, hist: 0 };
    const series = [];
    for (let i = slow - 1; i < a.length; i++) {
      const slice = a.slice(0, i + 1);
      const fastEma = ema(slice, fast);
      const slowEma = ema(slice, slow);
      series.push(fastEma - slowEma);
    }
  ...
  for (const tf of ["15m", "1h", "4h", "1d"]) {
    try {
      ...
      // EMA cross slope (change in fast‑minus‑slow diff from prev candle)
      const ema50Prev = ema(closes.slice(0, -1), 50);
      const ema200Prev = ema(closes.slice(0, -1), 200);
      const diffNow = ema50 - ema200;
      const diffPrev = ema50Prev - ema200Prev;
      const emaCrossSlope = +((diffNow - diffPrev).toFixed(2));
  ...
  for (const tf of ["15m", "1h", "4h", "1d"]) {
    try {
      ...
      let note;
      if (pct >= 1.5)
        note = "strong up-move – breakout long / exit shorts";
      else if (pct >= 0.5) note = "bullish drift – long bias";
      else if (pct <= -1.5)
        note = "strong down-move – breakout short / exit longs";
  ...
  try {
    ...
    const up =
      cg.sentiment_votes_up_percentage ?? cg.community_data?.sentiment_votes_up_percentage;
